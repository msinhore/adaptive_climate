...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def async_set_parameter(call: ServiceCall) -> None:
        ...
        if not coordinator:
            _LOGGER.error(
                "Could not find coordinator for entity %s", entity_id
            )
            return

        device_name = coordinator.device_name
        _LOGGER.debug(
            f"[{device_name}] Setting parameter: {parameter} = {value}"
        )
        ...
        try:
            validated_value = _validate_parameter_value(
                parameter, value, param_def
            )
        except ValueError as err:
            _LOGGER.error(
                f"[{device_name}] Invalid value for parameter {parameter}: {err}"
            )
            return
        ...
        _LOGGER.info(
            f"[{device_name}] Parameter {parameter} updated to {validated_value}"
        )
    ...
    async def async_reset_parameter(call: ServiceCall) -> None:
        ...
        _LOGGER.info(
            f"[{device_name}] Parameter {parameter} reset to default: {default_value}"
        )
    ...
    async def async_set_temporary_override(call: ServiceCall) -> None:
        """Handle set temporary override service call."""
        entity_id = call.data["entity_id"]
        ...
        device_name = coordinator.device_name
        _LOGGER.error(
            f"[{device_name}] Temporary override service has been removed. Use auto_mode_enable to control manual/automatic mode."
        )

        _LOGGER.info(
            f"[{device_name}] Temporary override service is no longer available"
        )
    ...
    async def async_redetect_capabilities(call: ServiceCall) -> None:
        ...
        _LOGGER.info(
            f"[{device_name}] Device capabilities re-detected: {capabilities}"
        )
    ...
    services_to_register = [
        ...
        (
            ...
            async_reset_parameter,
            RESET_PARAMETER_SCHEMA,
        ),
        (
            SERVICE_SET_COMFORT_CATEGORY,
            async_set_comfort_category,
            SET_COMFORT_CATEGORY_SCHEMA,
        ),
        (
            SERVICE_UPDATE_CALCULATIONS,
            async_update_calculations,
            UPDATE_CALCULATIONS_SCHEMA,
        ),
        (
            SERVICE_SET_TEMPORARY_OVERRIDE,
            async_set_temporary_override,
            SET_TEMPORARY_OVERRIDE_SCHEMA,
        ),
        (
            SERVICE_REDETECT_CAPABILITIES,
            async_redetect_capabilities,
            REDETECT_CAPABILITIES_SCHEMA,
        ),
    ]

    for service_name, service_func, schema in services_to_register:
        hass.services.async_register(
            DOMAIN, service_name, service_func, schema=schema
        )
...
def _validate_parameter_value(
    parameter: str, value: Any, param_def: dict
) -> Any:
    """
    ...
    try:
        if param_def["type"] == float:
            ...
            if "min" in param_def and converted_value < param_def["min"]:
                raise ValueError(
                    f"Value {converted_value} below minimum {param_def['min']}"
                )
            if "max" in param_def and converted_value > param_def["max"]:
                raise ValueError(
                    f"Value {converted_value} above maximum {param_def['max']}"
                )
            ...
            if (
                "options" in param_def
                and converted_value not in param_def["options"]
            ):
                raise ValueError(
                    f"Value {converted_value} not in allowed options: {param_def['options']}"
                )
            return converted_value
...
async def _get_coordinator_from_entity_id(
    hass: HomeAssistant, entity_id: str
) -> Optional[AdaptiveClimateCoordinator]:
    """
    ...
    for entry_id, coordinator in coordinators.items():
        if isinstance(coordinator, AdaptiveClimateCoordinator):
            ...
            if entity_name.startswith(entry_id):
                _LOGGER.debug(
                    f"Found coordinator for entity {entity_id}: {coordinator.device_name}"
                )
                return coordinator
